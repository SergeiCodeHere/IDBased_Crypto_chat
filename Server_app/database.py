...
import sqlite3
import logging
from db_structure import QUERY_user
...
class Database:
    ...
    def setup(self):
        ...
        if not self.is_database_ready():
            if self.filename == 'db_konf_s.sqlite':
                return False
    ...
    def is_database_ready(self):
        ...
        try:
            if self.filename == 'db_konf_s.sqlite':
                self.cur.execute("SELECT id FROM SERVER_CONF LIMIT 1")
                return True
        except:
    ...
    def create_structure(self):
        if self.filename == 'db_user.sqlite':
            self.cur.executescript(QUERY_user)
        self.con.commit()
